...
### 5.3 Codecs — surveyed 2026-08-31, not yet implemented (next up)
- [ ] Shim `StreamCodec`, `ByteBufCodecs`, `NeoForgeStreamCodecs`, `BackportCodecs`, `NBTCodecHelper` behavior over `FriendlyByteBuf` (1.20.1 has no `RegistryFriendlyByteBuf` — plain `FriendlyByteBuf` is the right buffer type) + DataFixerUpper `Codec`. Most are thin functional wrappers → straightforward, but bigger than it first looks: **126 compiler errors** trace to `StreamCodec` alone, more than any other single missing symbol.
- [ ] **Important shape note**: NeoBackports' `StreamCodec` is **single-type-parameter** (`StreamCodec<V>`, buffer type fixed internally) — confirmed from `FishCaughtPayload.STREAM_CODEC : StreamCodec<FishCaughtPayload>` — not real NeoForge's 2-param `StreamCodec<B, V>`. Match that shape, don't reproduce upstream NeoForge's actual API.
- [ ] Exact member survey (`grep -rhoE "ByteBufCodecs\.[A-Za-z_]+" src/main/java`), so nothing gets missed:
  - `ByteBufCodecs`: `INT` (22), `INGREDIENT` (18), `FLOAT` (12), `BOOL` (12), `STRING` (11), `list(...)` (9), `RESOURCE_LOCATION` (6), `holderRegistry(...)` (4, registry-aware — needs a resolvable `Registry`/`Holder`, so depends on §5.1 being in place, which it now is), `VAR_INT` (4), `UUID` (4), `ITEM_STACK` (4), `fromCodec(...)` (3, wraps a DFU `Codec<T>` into a `StreamCodec<T>`), `VAR_LONG` (3), `STRING_UTF` (2), `map(...)` (1).
  - `NeoForgeStreamCodecs`: only `.enumCodec(Class<E>)` (5 sites) — small, do first.
  - `BackportCodecs`: `IngredientCodecs` (9, likely a nested holder of ingredient-related codecs — inspect call sites before designing), `ITEM_STACK_RECIPE` (2), `strictUnboundedMap(...)` (1).
  - `StreamCodec.composite(...)` call sites span 15 files (payloads, attachments, `SingleStackContainer`, `FishProperties`, `LetterItem`, `TournamentSettings`) with varying arity — the real API needs composite overloads for at least 1 through 6 fields (matches every site seen so far).
- [ ] Suggested build order: `NeoForgeStreamCodecs.enumCodec` → `StreamCodec` core + `composite` (start with low arity, extend as needed) → `ByteBufCodecs` primitives (INT/FLOAT/BOOL/STRING/VAR_INT/VAR_LONG/UUID/RESOURCE_LOCATION) → `ByteBufCodecs.list`/`fromCodec` → `ByteBufCodecs.INGREDIENT`/`ITEM_STACK`/`holderRegistry` (these need real vanilla codec plumbing, save for last) → `BackportCodecs`.
...
2. **P1 Shim core** — §5.1 registries/Deferred* ✅ done (2026-08-31): `SCItems`/`SCBlocks`/`SCBlockEntities`/`SCEntities`/`SCSounds`/`SCParticles`/`SCMenuTypes`/`SCCreativeModeTabs`/`SCCriterionTriggers` all wired into `StarcatcherFabric.onInitialize()` in dependency order and verified error-free by the real compiler. §5.3 codecs surveyed but not yet built (next up — see §5.3 for the exact member list and suggested build order); §5.8 utils not started.
