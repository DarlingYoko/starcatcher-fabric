...
2. **P1 Shim core** — §5.1 registries/Deferred* ✅ and §5.3 codecs ✅, both done (2026-08-31) and verified against the real compiler (full-project error count 3554→682 lines of output). `SCItems`/`SCBlocks`/`SCBlockEntities`/`SCEntities`/`SCSounds`/`SCParticles`/`SCMenuTypes`/`SCCreativeModeTabs`/`SCCriterionTriggers` wired into `StarcatcherFabric.onInitialize()`. §5.8 misc utils not started (next up).
